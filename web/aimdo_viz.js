...
function renderModelRow(r, m, data) {
    ...
    if (row.lastDynamic !== m.dynamic) {
        row.bar.innerHTML = "";
        row.barSegs = [];
        for (const color of (m.dynamic ? [vramColor, C.pinned, C.unloaded] : [vramColor, C.pinned])) {
    ...
    row.barSegs[0].style.background = vramColor;
    if (row.barSegs[1]) row.barSegs[1].style.background = C.pinned;
    if (row.barSegs[2]) row.barSegs[2].style.background = C.unloaded;
